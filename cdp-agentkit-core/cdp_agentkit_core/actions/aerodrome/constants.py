...
AERODROME_FACTORY_CONTRACT_ADDRESSES = {
...
def get_factory_address(network: str) -> str:
    ...
    if network not in AERODROME_FACTORY_CONTRACT_ADDRESSES:
        raise ValueError(
            f"Invalid network: {network}. Valid networks are: {', '.join(AERODROME_FACTORY_CONTRACT_ADDRESSES.keys())}"
        )
    return AERODROME_FACTORY_CONTRACT_ADDRESSES[network]
